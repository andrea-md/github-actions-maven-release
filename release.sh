...
mvn -B $MAVEN_SETTINGS_OPTION $MAVEN_REPO_LOCAL -Dusername=$GITHUB_ACTOR release:branch -DbranchName=v$MAVEN_RELEASE_VERSION.x -DautoVersionSubmodules=true -DupdateBranchVersions=true -DupdateWorkingCopyVersions=false -DreleaseVersion=$MAVEN_RELEASE_VERSION.0-SNAPSHOT
